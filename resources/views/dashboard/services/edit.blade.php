...
@section("container")
    ...
    <div class="font-jost ml-0 lg:ml-80 mt-16 lg:mt-24 p-5">
        <h1 class="font-bold text-xl md:text-3xl pb-5">Editing: {{ $service->title }}</h1>

        <form onsubmit="return confirm('Do you really want to update this service?');" method="POST" action="/dashboard/services/{{ $service->slug }}" class="w-full lg:w-1/2">
            @csrf
            ...
            <div class="flex flex-col">
                ...
                <input value="{{ $service->title }}" class="p-2 border border-gray-300 rounded-lg" type="text" name="title" id="title" placeholder="Make an interesting title..." required>
            </div>

            <div class="flex flex-col">
                <label class="text-lg py-2">Description</label>
                <input value="{{ $service->desc }}" id="desc" type="hidden" name="desc">
                <trix-editor class="trix-content" input="desc"></trix-editor>
            </div>

            {{-- <div class="flex mt-5 items-center">
                @if($service->isArchived == '0')
