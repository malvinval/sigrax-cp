...
@section("container")
  ...
  <div class="font-jost ml-0 lg:ml-80 mt-16 lg:mt-24 p-5">
    <div class="flex flex-col w-full lg:w-1/3 pb-5">
        ...
        <form class="flex" method="GET" action="/dashboard/home">             
    ...
    <div>
        @if($contents->count() > 0)
            @if($edit_mode === "0")
                <a class="underline font-bold px-5 mr-1 py-2 bg-yellow-500 hover:bg-yellow-600 text-gray-900 rounded-lg" href="/dashboard/home?section={{ $section }}&edit_mode=1">Enter edit mode</a>
            @else
                <a class="px-5 mr-1 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg" href="/dashboard/home?section={{ $section }}">Finish editing</a>
            @endif
                <div class="rounded-lg py-5">
                @include("templates.home.section$section", compact(
                    "contents", "edit_mode"
                ))
            </div>
        @else
