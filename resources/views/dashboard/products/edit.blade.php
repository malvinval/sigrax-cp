...
@section("container")
    ...
    <div class="font-jost ml-0 lg:ml-80 mt-16 lg:mt-24 p-5">
        <h1 class="font-bold text-xl md:text-3xl pb-5">Editing: {{ $product->title }}</h1>

        <form onsubmit="return confirm('Do you really want to update this product?');" method="POST" action="/dashboard/products/{{ $product->slug }}" class="w-full lg:w-1/2">
            @csrf
            ...
            <div class="flex flex-col">
                ...
                <input value="{{ $product->title }}" class="p-2 border border-gray-300 rounded-lg" type="text" name="title" id="title" placeholder="Make an interesting title..." required>
            </div>

            <div class="flex flex-col">
                <label class="text-lg py-2">Description</label>
                <input value="{{ $product->desc }}" id="desc" type="hidden" name="desc">
